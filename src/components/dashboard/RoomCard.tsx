...
export function RoomCard({ room }: RoomCardProps) {
  ...
  return (
    <div
      ...
      className={`
        bg-white border border-gray-100 border-l-[3px] ${statusBorderColors[room.status] || "border-l-gray-200"}
        rounded-xl overflow-hidden transition-all duration-150
        ${isOccupied ? "cursor-pointer hover:shadow-md hover:border-gray-200" : ""}
      `}
    >
      <div className="px-4 py-3.5">
        {/* Room number + status */}
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-semibold text-gray-400">Room {room.number}</span>
        ...
        {patient ? (
          <>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-gray-400">{patient.patientCode}</p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-center py-4">
            <div className="text-center">
              <div className="w-8 h-8 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-1.5">
                <svg className="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
              </div>
              <p className="text-[10px] text-gray-300 font-medium">Available</p>
            </div>
          </div>
        )}
      </div>
