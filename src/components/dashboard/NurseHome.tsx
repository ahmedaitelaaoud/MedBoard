...
export function NurseHome({ user }: { user: SessionUser }) {
  ...
  const renderTaskCard = (task: TaskItem) => {
    ...
    return (
      <div
        key={task.id}
        className={`
          rounded-xl border px-3.5 py-3 bg-white dark:bg-slate-900 shadow-xs transition-colors
          ${isCompleted ? "border-emerald-100 dark:border-emerald-900/40 opacity-70" : "border-slate-200 dark:border-slate-700"}
        `}
      >
        <div className="flex items-start justify-between gap-2">
          <p className={`text-sm font-semibold ${isCompleted ? "text-gray-500 dark:text-slate-500 line-through" : "text-slate-700 dark:text-slate-100"}`}>
            {task.title}
          </p>
          <Badge
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{formatTaskTime(task.createdAt)}</span>
          ...
          {isCompleted ? (
            ...
            <button
              ...
              className="text-[11px] px-2.5 py-1 rounded-full border border-emerald-200 dark:border-emerald-900/60 text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-900/20 hover:bg-emerald-100 dark:hover:bg-emerald-900/35 transition-colors"
            >
              Mark done
            </button>
          ) : (
            <button
              ...
              className="text-[11px] px-2.5 py-1 rounded-full border border-amber-200 dark:border-amber-900/60 text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/20 hover:bg-amber-100 dark:hover:bg-amber-900/35 transition-colors"
            >
              To Do
            </button>
          )}
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Daily task board */}
      <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-100/80 dark:bg-slate-900 p-3.5 sm:p-4 transition-colors duration-200">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h2 className="text-2xl font-semibold text-slate-800 dark:text-slate-100">Daily task board</h2>
            <p className="text-[11px] uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400 mt-1">Tasks board for the day</p>
          </div>
                ...
                className={`
                  ...
                  ${
                    priorityFilter === filter
                      ? "bg-slate-900 dark:bg-brand-700 text-white border-slate-900 dark:border-brand-600"
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="text-[10px] uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">Tâches du jour</div>
          <div className="text-xs text-slate-500 dark:text-slate-400 capitalize">{boardDateLabel}</div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
          {boardColumns.map((column) => (
            <div key={column.key} className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-200/60 dark:bg-slate-800/60 p-2 transition-colors">
              <div className="rounded-lg bg-slate-300/80 dark:bg-slate-700 px-3 py-2.5 text-center">
                <p className="text-sm font-semibold tracking-wide text-slate-700 dark:text-slate-200">{column.title}</p>
                <p className="text-[10px] mt-0.5 tracking-wide text-slate-600 dark:text-slate-400">{column.subtitle}</p>
              </div>

              <div className="mt-2 space-y-2 min-h-[260px]">
                {column.tasks.length === 0 ? (
                  <div className="h-[42px] rounded-lg border border-dashed border-slate-300 dark:border-slate-600 bg-white/70 dark:bg-slate-900/40 flex items-center justify-center text-xs text-slate-500 dark:text-slate-400">
                    No task
                  </div>
                ) : (
